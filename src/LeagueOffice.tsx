...
export default function LeagueOffice() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const d = await window.api.getLeagueOfficeData();
    setData(d);
    setLoading(false);
  }, []);
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            {data.userVote && (
              <span style={{ fontSize: 11, color: T.textDim, marginLeft: 4 }}>
                Stance recorded for this offseason
              </span>
            )}
          </div>
        </div>
          ...
          <div>
            ...
            {data.recentExpansions.map((e: any, i: number) => (
              <div key={i} style={{
                ...
                background: T.bgCard, border: `1px solid ${T.borderFaint}`, borderRadius: 4,
                fontSize: 12,
              }}>
                ...
                <span style={{ color: '#66BB6A', marginLeft: 10, fontSize: 11 }}>
                  {e.votes_for}–{e.votes_against}
                </span>
              </div>
            ))}
          </div>
      ...
      {showRelocationModal && (
        <div style={{
          ...
          <div style={{
            ...
            borderRadius: 8, padding: 28, minWidth: 400, maxWidth: 500,
            ...
            {relocationCities.length === 0 ? (
              ...
              <>
                ...
                <div style={{ maxHeight: 240, overflowY: 'auto', marginBottom: 16 }}>
                  {relocationCities.map(city => (
                    <div key={city.city} onClick={() => setSelectedCity(city)} style={{
                      ...
                      background: selectedCity?.city === city.city
                        ? 'rgba(255,135,64,0.15)' : 'transparent',
                ...
                {relocationMsg && (
                  <div style={{ color: '#e57373', fontSize: 12, marginBottom: 10 }}>{relocationMsg}</div>
                )}
                <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                  ...
                  <button onClick={confirmRelocation} disabled={!selectedCity} style={{
                    ...
                    borderRadius: 4, cursor: selectedCity ? 'pointer' : 'default',
