...
const EVENT_META: Record<string, { icon: string; color: string }> = {
  ...
  award:           { icon: '🏅', color: '#FFD700' },
  contract:        { icon: '📝', color: '#aaa'    },
  contract_demand: { icon: '💰', color: '#FF8740' },
};
...
export default function NewsFeed() {
  ...
  useEffect(() => {
    ...
    window.api.getNewsUnseenCount?.(viewSeason).then((n: number) => setUnseenCount(n)).catch(() => {});
  }, [viewSeason, category]);

  const handleMarkSeen = () => {
    if (viewSeason) window.api.markNewsSeen?.(viewSeason).then(() => setUnseenCount(0)).catch(() => {});
  };
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 860, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {unseenCount > 0 && (
            <button onClick={handleMarkSeen} style={{
              fontSize: 10, color: '#FF8740', background: '#1a0a00', border: '1px solid #FF874033',
              borderRadius: 4, padding: '3px 10px', cursor: 'pointer',
            }}>
              {unseenCount} new league events — mark seen
            </button>
          )}
          {seasons.length > 1 && (
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 20, flexWrap: 'wrap' }}>
        {CATEGORIES.map(c => {
          const isLeague = c.id === 'league';
          return (
            <button key={c.id} onClick={() => setCategory(c.id)} style={{
              padding: '4px 12px', fontSize: 11, fontWeight: 700, borderRadius: 4, cursor: 'pointer',
              background: category === c.id ? (isLeague ? '#4FC3F7' : '#FF8740') : T.bgCard,
              border: `1px solid ${category === c.id ? (isLeague ? '#4FC3F7' : '#FF8740') : T.borderFaint}`,
              color: category === c.id ? '#000' : T.textMuted,
              position: 'relative' as const,
            }}>
              {c.label}
              {isLeague && unseenCount > 0 && category !== 'league' && (
                <span style={{
                  position: 'absolute' as const, top: -4, right: -4,
                  background: '#FF8740', color: '#000', borderRadius: '50%',
          ...
          events.map(event => {
            ...
            return (
              <div key={event.id} onClick={() => hasDetail && setExpandedId(isExpanded ? null : event.id)}
                ...
                <div style={{ fontSize: 18, width: 28, textAlign: 'center', paddingTop: 1 }}>{meta.icon}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: meta.color }}>{event.headline}</span>
                    {hasDetail && <span style={{ fontSize: 10, color: T.textDim }}>{isExpanded ? '▲' : '▼'}</span>}
                  </div>
                  {isExpanded && hasDetail && (
                    <div style={{ fontSize: 12, color: T.textMuted, marginTop: 4, lineHeight: 1.5 }}>{event.detail}</div>
                  )}
