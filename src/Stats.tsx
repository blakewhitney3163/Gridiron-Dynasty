  ...
  return (
    <div style={{ padding: '20px 24px', maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
        <div style={{ flex: 1 }}>
          <h2 style={{ color: T.textPrimary, fontSize: 18, fontWeight: 700, margin: 0 }}>
            {viewMode === 'teams'
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button onClick={handleImport} style={{ padding: '4px 10px', fontSize: 10, background: T.bgCard, border: `1px solid ${T.borderFaint}`, borderRadius: 3, color: T.textDim, cursor: 'pointer' }}>
      ...
      {viewMode === 'teams' && (
        <TeamStatsTable data={teamSeasonStats ?? []} sortKey={teamSortKey} sortDir={teamSortDir} onSort={handleTeamSort} />
      )}

      {viewMode === 'players' && (
        <>
          {category === 'passing'       && <PassingTable       rows={passing}          onSelect={setSelectedPlayer} />}
          {category === 'rushing'       && <RushingTable       rows={rushing}          onSelect={setSelectedPlayer} />}
          {category === 'receiving'     && <ReceivingTable     rows={receiving}        onSelect={setSelectedPlayer} />}
          {category === 'defense'       && <DefenseTable       tackles={tackles} sacks={sacks} defInts={defInterceptions} subCat={defSubCat} onSubCat={setDefSubCat} onSelect={setSelectedPlayer} />}
          {category === 'special_teams' && <SpecialTeamsTable  rows={kickers}          onSelect={setSelectedPlayer} />}
        </>
      )}
    </div>
  );
