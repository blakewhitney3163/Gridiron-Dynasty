...
export const OFFENSE_SCHEMES = [
  {
    id: 'West Coast',
    name: 'West Coast',
    tagline: 'Short passes, quick rhythm',
    keyPositions: 'QB (Accuracy) · TE',
    description: 'Favors an accurate QB and a receiving TE. High completion rates and sustained drives.',
  },
  {
    id: 'Air Raid',
    name: 'Air Raid',
    tagline: 'Throw it everywhere',
    keyPositions: 'WR (Top 3)',
    description: 'Pass-heavy, spread formations. Elite WR corps unlock the full potential of this system.',
  },
  {
    id: 'Power Run',
    name: 'Power Run',
    tagline: 'Pound the rock',
    keyPositions: 'RB · OL',
    description: 'Run-first identity built around a workhorse RB and a dominant offensive line.',
  },
  {
    id: 'Spread',
    name: 'Spread',
    tagline: 'Space and mobility',
    keyPositions: 'QB (Speed + Accuracy)',
    description: 'Mobile QB who stresses defenses with both legs and arm. Speed is everything.',
  },
  {
    id: 'Run & Gun',
    name: 'Run & Gun',
    tagline: 'Up-tempo, balanced attack',
    keyPositions: 'All Offensive Positions',
    description: 'Fast-paced, balanced run/pass. Rewards deep and well-rounded rosters.',
  },
];
...
export const DEFENSE_SCHEMES = [
  {
    id: '4-3',
    name: '4-3',
    tagline: 'Four down linemen',
    keyPositions: 'DL (Top 4)',
    description: 'Base four-man front. Strong vs. the run with consistent pressure from the line.',
  },
  {
    id: '3-4',
    name: '3-4',
    tagline: 'Linebacker-heavy',
    keyPositions: 'LB (Top 4)',
    description: 'Three linemen, four linebackers. Elite LBs generate both coverage and pass rush.',
  },
  {
    id: 'Zone Cover 2',
    name: 'Zone Cover 2',
    tagline: 'Two-deep zone coverage',
    keyPositions: 'CB · S (Coverage)',
    description: 'Limits big plays with a two-deep safety shell. Best when DBs have elite coverage.',
  },
  {
    id: 'Man Press',
    name: 'Man Press',
    tagline: 'Press and jam at the line',
    keyPositions: 'CB (Top 2)',
    description: 'Physical corners that shut down receivers man-to-man. High ceiling with elite CBs.',
  },
  {
    id: 'Blitz Heavy',
    name: 'Blitz Heavy',
    tagline: 'Extra rushers, maximum pressure',
    keyPositions: 'DL + LB (Pass Rush)',
    description: 'Sends extra pass rushers every down. Elite pass rush dominates; weak one leaves gaps.',
  },
];
...
export function registerSchemeHandlers(): void {
  ipcMain.handle('get-team-scheme', (_event: any, teamId: number) => {
    const row = db.prepare('SELECT offense_scheme, defense_scheme FROM team_schemes WHERE team_id = ?').get(teamId) as any;
    return {
  ...
  ipcMain.handle('get-scheme-options', (_event: any, teamId: number) => {
    const row = db.prepare('SELECT offense_scheme, defense_scheme FROM team_schemes WHERE team_id = ?').get(teamId) as any;
    ...
    return {
      currentOff,
      currentDef,
      offenseOptions: OFFENSE_SCHEMES.map(s => ({
        ...s,
        fit: computeOffenseFit(teamId, s.id),
        current: s.id === currentOff,
      })),
      defenseOptions: DEFENSE_SCHEMES.map(s => ({
        ...s,
        fit: computeDefenseFit(teamId, s.id),
        current: s.id === currentDef,
      })),
    };
  });

  ipcMain.handle('set-team-scheme', (_event: any, payload: { teamId: number; offenseScheme?: string; defenseScheme?: string }) => {
    const { teamId, offenseScheme, defenseScheme } = payload;
    const current = db.prepare('SELECT offense_scheme, defense_scheme FROM team_schemes WHERE team_id = ?').get(teamId) as any;
