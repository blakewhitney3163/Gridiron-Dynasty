...
export default function OffseasonChecklist({
    ...
    switch (offseasonPhase) {
      ...
      case 'resign': return (
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button style={navBtn} onClick={() => onNavigate('myteam')}>→ Go to Re-Signs</button>
            <button style={primaryBtn('#4caf50', faOpening)} onClick={handleOpenFa} disabled={faOpening}>
      ...
      case 'fa_week1': return (
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button style={navBtn} onClick={() => onNavigate('myteam')}>→ Browse Free Agents</button>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
            <div style={{ padding: '8px 16px', background: '#0d0d0d', borderRadius: 4, border: `1px solid ${T.borderFaint}` }}>
              <div style={{ fontSize: 9, color: '#444', letterSpacing: 0.5, marginBottom: 3 }}>DRAFT CLASS</div>
              <div style={{ fontSize: 11, color: T.textSecondary }}>
                {draftGenerated ? 'Generated — ready to scout' : 'Not yet generated'}
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button style={navBtn} onClick={() => onNavigate('draft')}>→ Scout Prospects</button>
            <button style={primaryBtn('#FF8740', advancing)} onClick={handleAdvance} disabled={advancing}>
      ...
      case 'roster_review': {
        ...
        return (
          <div>
            ...
            {overLimit ? (
              <button
                style={{ padding: '8px 18px', fontSize: 11, fontWeight: 700, cursor: 'pointer', background: '#3a0a0a', border: '1px solid #e5737355', color: '#e57373', borderRadius: 4 }}
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, padding: '8px 12px', background: '#060e06', borderRadius: 4, border: '1px solid #4caf5033' }}>
                  <span style={{ fontSize: 12, color: '#4caf50' }}>✓</span>
                  <span style={{ fontSize: 11, color: '#4caf50' }}>
                    Roster set — use the Advance Season button in the sidebar when ready
                  </span>
                </div>
                <button style={{ padding: '7px 14px', fontSize: 11, cursor: 'pointer', borderRadius: 4, background: '#111', border: `1px solid ${T.borderMid}`, color: T.textMuted }} onClick={() => onNavigate('myteam')}>
                  → Review Roster
                </button>
              </div>
            )}
  ...
  return (
    <div style={{ background: T.bgPanel, border: `1px solid ${T.borderMid}`, borderRadius: 8, padding: '16px 20px' }}>
      ...
      <div style={{ background: '#0a0a0a', border: '1px solid #FF874022', borderLeft: '3px solid #FF8740', borderRadius: 6, padding: '16px 18px' }}>
        {phaseContent}
      </div>
    </div>
  );
